...
import { useCopyToClipboard } from '@st/hooks/useCopyToClipboard';
...
export const IconsDisplay = () => {
	...
	const filteredIcons = allIcons.filter(([name]) => name.toLowerCase().includes(searchTerm.toLowerCase()));

	const handleIconClick = (name: string) => {
		const svgTemplate = `<Icon icon={${name}} />`;

		copyToClipboard({ text: svgTemplate, message: 'Icon copied to the clipboard!' });
	};
			...
			<div style={{ fontFamily: 'Arial, sans-serif' }}>
				...
				<div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '1rem' }}>
					{filteredIcons.map(([name, icon]: [name: string, icon: any]) => {
						if (typeof icon.$$typeof === 'symbol' && icon.$$typeof.toString() === 'Symbol(react.element)') {
							return (
								<div key={name} style={{ textAlign: 'center' }}>
									...
									>
										<div style={{ marginBottom: '0.5rem' }}>{RenderSvgIcon({ icon })}</div>
